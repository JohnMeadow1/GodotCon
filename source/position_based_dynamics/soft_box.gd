...
func _ready():
	generate_soft_box_body( verticles, verticles_distance )
...
func generate_soft_box_body( nodes:int, size:float )->void:
	var distance_from_ground = Vector2(0, 600 - (position.y + nodes * size) )
	for i in range(nodes):
		for j in range(nodes):
#			if i == 0:
#				add_vertex(  Vector2( j, i ) * size + distance_from_ground, true , 1.0)
#			else:
				add_vertex(  Vector2( j, i ) * size + distance_from_ground, false , 1.0)
